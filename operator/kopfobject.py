...
class KopfObject:
    ...
    async def fetch_definition(cls, name, namespace):
        return await Replik8s.custom_objects_api.get_namespaced_custom_object(
            group = cls.api_group,
            name = name,
            namespace = namespace,
            plural = cls.plural,
    ...
    async def list_definitions(cls, namespace, label_selector=None):
        _continue = None
        while True:
            obj_list = await Replik8s.custom_objects_api.list_namespaced_custom_object(
                group = cls.api_group,
    ...
    async def delete(self):
        try:
            definition = await Replik8s.custom_objects_api.delete_namespaced_custom_object(
    ...
    async def merge_patch(self, patch):
        definition = await Replik8s.custom_objects_api.patch_namespaced_custom_object(
            group = self.api_group,
            name = self.name,
            namespace = self.namespace,
            plural = self.plural,
    ...
    async def merge_patch_status(self, patch):
        definition = await Replik8s.custom_objects_api.patch_namespaced_custom_object_status(
            group = self.api_group,
            name = self.name,
            namespace = self.namespace,
            plural = self.plural,
    ...
    async def replace(self, definition):
        updated_definition = await Replik8s.custom_objects_api.replace_namespaced_custom_object(
            group = self.api_group,
            name = self.name,
            namespace = self.namespace,
            plural = self.plural,
